...
# Adaptation of original code from HuggingFace's transformers library to
# incrementally build Arguments objects to allow for subclassing
import contextlib
...
class Base(object):
    '''Base class to be inherited by all arguments mixin classes'''
...
class CommonArgsMixin(Base):
    '''
    Mixin class for arguments always used in config classes.
...
@dataclass
class ModelArgsMixin(Base):
    '''
    Mixin class for arguments used in model training.
...
@dataclass
class LoggingArgsMixin(Base):
    '''
    Mixin class for arguments related to logging.
...
@dataclass
class DataloaderArgsMixin(Base):
    '''
    Mixin class for arguments related to dataloaders.
...
@dataclass
class PushToHubArgsMixin(Base):
    '''
    Mixin class for arguments related to pushing to the model hub.
...
@dataclass
class HyperParamSearchArgsMixin(Base):
    '''
    Mixin class for arguments related to hyperparameter search.
...
class ModelTrainingArguments(
    ...
    '''
    ModelTrainingArguments is the subset of the arguments we use in our example scripts **which relate to the model itself**.
    ...
    def update_from_global_args(self, global_args: TrainingArguments) -> 'ModelTrainingArguments':
        ...
        Parameters:
            global_args (`[TrainingArguments]`):
                The global training arguments to update from.
        '''
